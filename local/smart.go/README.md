# smart.go (Local Fork for niceblocks)

**This is a local fork of [github.com/anatol/smart.go](https://github.com/anatol/smart.go) used by the niceblocks project.**

## Local Changes

### Fix SATA detection for USB bridges (sata_linux.go)

**Problem:** When SATA drives are connected over USB, many bridges don't properly report `"ATA     "` in the SCSI INQUIRY VendorIdent field. Instead, they report the actual drive manufacturer (e.g., `"TOSHIBA "`). This caused the library to detect these drives as generic SCSI devices, which don't support SMART reading.

**Solution:** Instead of relying solely on VendorIdent matching, the fix now:
1. Checks if the device is a direct-access block device (required for SAT)
2. If VendorIdent is `"ATA     "`, proceeds directly (fast path)
3. Otherwise, **tests** if the device responds to ATA IDENTIFY command
4. If ATA IDENTIFY succeeds, treats it as a SATA/SAT device
5. If it fails, returns an error (falling back to pure SCSI)

This approach is more robust as it actually tests device capabilities rather than relying on brittle string matching.

### Usage in niceblocks

The main project uses this local fork via Go's replace directive in `go.mod`:

```
replace github.com/anatol/smart.go => ./local/smart.go
```

### Pushing to GitHub / Creating PR

When ready to push this fork:

```bash
cd /home/robin/projects/niceblocks/local/smart.go
git init
git add .
git commit -m "Fix SATA detection for USB bridges that don't report 'ATA     ' in VendorIdent"
...
Then submit a PR to [github.com/anatol/smart.go](https://github.com/anatol/smart.go) - this fix benefits all users with USB-to-SATA bridges.

---

## Original smart.go Documentation
